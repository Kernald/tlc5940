...
static void Tlc_init(uint16_t initialValue = 0);
static void Tlc_clear(void);
static void Tlc_clearRow(uint8_t row);
static uint16_t Tlc_get(uint8_t row, TLC_CHANNEL_TYPE channel);
static void Tlc_set(uint8_t row, TLC_CHANNEL_TYPE channel, uint16_t value);
static void Tlc_setAll(uint16_t value);
static void Tlc_setRow(uint8_t row, uint16_t value);
static inline void Tlc_shiftRow(uint8_t row);
#if VPRG_ENABLED
static void Tlc_setAllDC(uint8_t value);
static void Tlc_dcModeStart(void);
static void Tlc_dcModeStop(void);
#endif
#if XERR_ENABLED
static uint8_t Tlc_readXERR(void);
#endif

static uint8_t tlc_GSData[NUM_ROWS][NUM_TLCS * 24];
...
/** Pin i/o and Timer setup.  The grayscale register will be reset to all
    ...
    \param initialValue = 0, optional parameter specifing the inital startup
           value */
static void Tlc_init(uint16_t initialValue)
{
    /* Pin Setup */
...
static uint16_t Tlc_get(uint8_t row, TLC_CHANNEL_TYPE channel)
{
    ...
    uint8_t *index12p = tlc_GSData[row] + ((((uint16_t)index8) * 3) >> 1);
